Feature: CollectFromCodebase — Pipeline Contexte Augmenté per Borough
  ...
  Scenario: CollectFromCodebase scoped — index fixtures + buildScoped + verify output file
    ...
    And the output file "build/context/test-borough.context.txt" contains "[RÈGLES_EAGER] Contexte de gouvernance"
  ...
  Scenario: CollectFromCodebase scoped — verify EAGER section contains INDEX content
    ...
    And the output file "build/context/test-borough-eager.context.txt" contains "[CONTEXTE_RAG] Résultats sémantiques"
  ...
  Scenario: CollectFromCodebase scoped — verify RAG section contains relevant results
    ...
    Then the output file "build/context/test-borough-rag.context.txt" contains "[CONTEXTE_RAG] Résultats sémantiques"
