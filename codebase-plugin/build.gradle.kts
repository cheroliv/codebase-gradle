...
dependencies {
    ...
    implementation(libs.bundles.langgraph4j)  // TODO(migrate → koog)
...
val cucumberTestEpicV1 = tasks.register<Test>("cucumberTestEpicV1") {
    description = "Runs Cucumber BDD tests — EPIC V-1 (VibecodingGraph) only"
    group = "verification"
    testClassesDirs = sourceSets.test.get().output.classesDirs
    classpath = configurations.testRuntimeClasspath.get() +
        sourceSets.test.get().output +
    ...
    filter { includeTestsMatching("codebase.scenarios.EpicV1CucumberRunner") }
}

val cucumberTestEpicV0 = tasks.register<Test>("cucumberTestEpicV0") {
    description = "Runs Cucumber BDD tests — EPIC V-0 (Custom tools shell) only"
    group = "verification"
    testClassesDirs = sourceSets.test.get().output.classesDirs
    classpath = configurations.testRuntimeClasspath.get() +
        sourceSets.test.get().output +
...
val cucumberTestEpicV4 = tasks.register<Test>("cucumberTestEpicV4") {
    description = "Runs Cucumber BDD tests — EPIC V-4 (Integration validation) only"
    group = "verification"
    testClassesDirs = sourceSets.test.get().output.classesDirs
    classpath = configurations.testRuntimeClasspath.get() +
        sourceSets.test.get().output +
    ...
    filter { includeTestsMatching("codebase.scenarios.EpicV4CucumberRunner") }
}
